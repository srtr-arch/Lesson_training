...
public class AdminLessonController {
    ...
    @PostMapping("/add")
    public String add(@Valid Lesson lesson,BindingResult result,Model model) {
        if(result.hasErrors()){
            List<Course> courses = courseService.findAllCourses();
        model.addAttribute("courses", courses);
    ...
    @GetMapping("/edit/{id}")
    public String editForm (@PathVariable("id") Long id, Model model)throws MyLessonNotFoundException {
        List<Course> courses = courseService.findAllCourses();
    ...
    @PostMapping("/edit")
    public String edit(Lesson lesson)throws MyLessonNotFoundException {
